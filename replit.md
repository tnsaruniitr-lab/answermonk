...
- **Competitor Playbook**: Report section analyzing top 3 competitors per segment — LLM-powered narrative analysis (GPT-4o-mini via `server/report/competitor-narrative.ts`), positioning themes (cross-engine consistency), verbatim AI quotes extracted via sentence-level analysis, authority sources with expandable links, social/community mentions (Reddit, Trustpilot etc.), and derived actionable recommendations. All 12 competitor narratives generated in parallel (~3.5s total). AI infrastructure domains (vertexaisearch.cloud.google.com) are labeled as "Gemini grounding" links.
