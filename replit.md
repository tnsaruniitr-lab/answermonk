...
- **Competitor Dedup**: Both report and teaser generators recount competitor appearances from raw_runs data (not stored pre-aggregated counts) to correctly merge name variants (e.g. "NEXA" / "Digital Nexa"). Groups all candidate name_raw values using `shouldMergeNames` fuzzy matching, then counts unique runs per group. This avoids double-counting when the same brand appears under different names in a single run. The scorer (`server/scoring/scorer.ts`) also has its own dedup for future scoring runs.
