...
- **Impact Summary Report**: A standalone summary page (`/summary/:id`) providing a high-impact overview of the full GEO analysis. Includes: Brand Visibility Scorecard (per-engine rates with visual bars), What Top Scorers Are Doing Right (top 3 competitors with full narrative, non-brand source URLs, social mentions, mention counts, avg rank, themes), Brand Mention Audit (brand vs competitor source counts by type — publications, social, directories), Biggest Gaps (weakest segments with verbatim AI quotes), Quick Wins (ranked actionable steps), and Authority Source Snapshot (top 25 domains with entity mentions and brand presence check).
