...
const BRAND_BAR = "linear-gradient(180deg, #1e1b4b 0%, #6366f1 55%, #10b981 100%)";

export default function CardVariantG() {
  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg, #ede9fe 0%, #ffffff 50%, #ecfdf5 100%)", padding: "28px 20px", fontFamily: "system-ui, -apple-system, sans-serif" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 18 }}>
        <div>
          <div style={{ fontSize: 9, fontWeight: 600, letterSpacing: "0.12em", color: "#ec4899", textTransform: "uppercase", marginBottom: 4 }}>Option G — Brand-Gradient Bar</div>
        ...
        {MOCK_TILES.map(tile => (
          <div key={tile.id} style={{
            ...
            onMouseEnter={e => {
              ...
              el.style.boxShadow = `0 6px 28px ${tile.accent}28, 0 1px 6px rgba(0,0,0,0.07)`;
          ...
          >
            {/* Left bar — single site brand gradient, identical across all cards */}
            <div style={{ width: 4, flexShrink: 0, background: BRAND_BAR }} />
                    ...
                    <div key={row.rank} style={{ position: "relative", marginBottom: 10 }}>
                      <div style={{
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: row.share > 0 ? 5 : 0 }}>
                          ...
                          {row.share > 0 && (
                            <span style={{ fontSize: 10.5, fontWeight: 800, flexShrink: 0, color: tile.accent, background: "rgba(255,255,255,0.7)", border: `1px solid ${tile.accent}35`, borderRadius: 20, padding: "2px 7px" }}>
                              {row.share}%
                            </span>
                          )}
                        </div>
