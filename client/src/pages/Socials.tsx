...
import { Globe, X, Plus, ChevronDown, Zap, BarChart2, FileText, AlertTriangle, ExternalLink, Loader2 } from "lucide-react";
...
type ResultTab = "videos" | "patterns" | "drafts";
...
export default function Socials() {
  ...
  const planMutation = useMutation({
    ...
    onMutate: () => { setPhase("loading"); setErrorMsg(""); setResult(null); },
    onSuccess: (data) => { setResult(data); setPhase("results"); setTab("drafts"); },
    onError: (err: any) => {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg, #ede9fe 0%, #ffffff 50%, #ecfdf5 100%)", fontFamily: "inherit" }}>
      ...
      <main style={{ maxWidth: 760, margin: "0 auto", padding: "48px 24px 80px" }}>
        ...
        {/* ── Brand Preview + Config ── */}
        {selectedBrand && phase !== "loading" && (
          <>
            {/* Brand preview */}
            <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 14, padding: "16px 18px", marginBottom: 20, display: "flex", gap: 14, alignItems: "flex-start", boxShadow: "0 1px 4px rgba(0,0,0,0.05)" }}>
        ...
        {phase === "loading" && (
          <div style={{ textAlign: "center", padding: "60px 0" }}>
            <div style={{ width: 56, height: 56, borderRadius: "50%", border: "3px solid rgba(99,102,241,0.2)", borderTopColor: "#6366f1", animation: "spin 0.9s linear infinite", margin: "0 auto 24px" }} />
            <div style={{ fontSize: 17, fontWeight: 700, color: "#111827", marginBottom: 8 }}>{STAGES[stageIdx]}</div>
            ...
            <div style={{ marginTop: 28, maxWidth: 360, margin: "28px auto 0" }}>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: "#9ca3af", marginBottom: 6 }}>
                {STAGES.slice(0, 4).map((s, i) => (
                  <span key={i} style={{ color: i <= stageIdx ? "#6366f1" : "#d1d5db", fontWeight: i === stageIdx ? 700 : 400 }}>●</span>
                ))}
              </div>
            </div>
          </div>
        ...
        {phase === "results" && result && (
          <div style={{ marginTop: 8 }}>
            {/* Metadata bar */}
            <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 20, padding: "12px 16px", background: "rgba(99,102,241,0.05)", borderRadius: 10, border: "1px solid rgba(99,102,241,0.1)" }}>
              {[
                { label: "Run time", val: `${result.runMetadata?.elapsedSec ?? "—"}s` },
                { label: "YouTube quota", val: result.runMetadata?.quotaUsed?.youtube?.toLocaleString() ?? "—" },
                { label: "Modes run", val: (result.runMetadata?.modesRan ?? []).join(", ") },
                { label: "Drafts", val: drafts.length },
                { label: "Videos", val: allVideos.length },
              ].map(m => (
                <div key={m.label} style={{ fontSize: 12 }}>
                  <span style={{ color: "#9ca3af" }}>{m.label}: </span>
                  <span style={{ fontWeight: 700, color: "#374151" }}>{m.val}</span>
                </div>
              ))}
              {result._mock && <span style={{ fontSize: 11, color: "#d97706", fontWeight: 700, marginLeft: "auto" }}>⚠ Sample data — live API pending</span>}
            </div>
            ...
            {/* Tabs */}
            <div style={{ display: "flex", gap: 0, background: "#f3f4f6", borderRadius: 10, padding: 4, marginBottom: 24 }}>
              {([
            ...
            {tab === "videos" && (
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                {allVideos.length === 0 ? <p style={{ color: "#9ca3af", textAlign: "center" }}>No videos returned.</p> : allVideos.map((v: any) => <VideoCard key={v.videoId} video={v} />)}
              </div>
            )}
            ...
            <div style={{ marginTop: 32, textAlign: "center" }}>
              <button
                ...
                style={{ padding: "10px 20px", background: "#fff", border: "1.5px solid #e5e7eb", borderRadius: 8, fontSize: 13, fontWeight: 700, color: "#6b7280", cursor: "pointer", fontFamily: "inherit" }}
              >
                ← Edit & re-run
              </button>
            </div>
