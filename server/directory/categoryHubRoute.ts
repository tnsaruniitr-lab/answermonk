...
function buildEditorialSummary(hub: HubData): { p1: string; p2: string } {
  ...
  if (topName && topPct) {
    ...
    p1 = `AnswerMonk has analysed ${hub.pageCount} ${catTitle} quer${hub.pageCount === 1 ? "y" : "ies"} in ${locTitle}. AI visibility rankings will appear here as data is collected.`;
  }
  ...
  if (authSources.length > 0) {
    ...
    p2 = `${authList} ${authSources.length === 1 ? "is" : "are"} among the most frequently cited authority sources when AI engines respond to ${catTitle} queries in ${locTitle}. AnswerMonk tracks rankings across all ${hub.pageCount} cluster variant${hub.pageCount === 1 ? "" : "s"} continuously, updating each time new prompt analysis is collected.`;
  } else {
    p2 = `AnswerMonk monitors ${hub.pageCount} ${catTitle} query variant${hub.pageCount === 1 ? "" : "s"} in ${locTitle} across ChatGPT, Claude, Gemini, and Perplexity. Rankings are updated each time new prompt analysis data is collected.`;
  }
...
<head>
  ...
  <title>Best ${catTitle} in ${locTitle} — AI Rankings | AnswerMonk</title>
  <meta name="description" content="AI search visibility rankings for ${hub.pageCount} ${catTitle} queries in ${locTitle}. Based on ${hub.totalBrandOccurrences} brand appearances analysed by AnswerMonk.">
...
<main>
  ...
  <div class="sub">AI Search Visibility — Category Hub · AnswerMonk</div>
