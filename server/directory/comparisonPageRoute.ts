...
function buildComparisonParagraph(d: ComparisonData): { p1: string; p2: string } {
  ...
  const p1 = `Across ${d.sharedPages.length} shared AI-analysed queries in ${loc}, ${leader} averages ${leaderPct} AI visibility compared to ${trailer}'s ${trailerPct} — a ${diffPct}-point gap based on Nexalytics GEO prompt analysis.`;
...
<head>
  ...
  <title>${d.brand1Name} vs ${d.brand2Name} in ${locTitle} — AI Visibility | Nexalytics GEO</title>
  <meta name="description" content="${d.brand1Name} averages ${avg1Pct} vs ${d.brand2Name}'s ${avg2Pct} AI visibility across ${d.sharedPages.length} shared queries in ${locTitle}. Nexalytics GEO.">
...
<main>
  ...
  <div class="sub">Prompt-level AI search visibility · Nexalytics GEO analysis</div>
